...
struct SettingsPanelView: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 20) {
          Text("个性化设置").font(.headline).frame(maxWidth: .infinity)

          settingSlider(
            title: "速度", value: $moveSpeed, range: 0...1, label: "\(Int(moveSpeed*100))%"
          ) { onSpeedChange() }
          settingSlider(title: "字号", value: $fontSize, range: 30...350, label: "\(Int(fontSize))")
          ...
          Button("修改字体样式") { withAnimation { showFontPicker = true } }
            .frame(maxWidth: .infinity).padding().background(Color.blue.opacity(0.15)).cornerRadius(
              12)

          Button("完成") { withAnimation { showSettings = false } }
            .bold().frame(maxWidth: .infinity).padding().background(Color.blue).foregroundColor(
              .white
            ).cornerRadius(12)
        }
        .padding(20)
    ...
    .frame(width: 320, height: 620)  // 稍微调高一点以容纳更多选项
    .background(
      ZStack {
        RoundedRectangle(cornerRadius: 30, style: .continuous).fill(.ultraThinMaterial)
        RoundedRectangle(cornerRadius: 30, style: .continuous).fill(Color(white: 0.15).opacity(0.8))
      }
    )
    .offset(
  ...
  private var dragHandle: some View {
    Capsule().fill(Color.gray.opacity(0.5)).frame(width: 40, height: 5).padding(.vertical, 12)
      .frame(maxWidth: .infinity).background(Color.clear).contentShape(Rectangle())
      .gesture(
        DragGesture().updating($interimOffset) { v, s, _ in s = v.translation }
          .onEnded { v in
  ...
  private func settingSlider(
    title: String, value: Binding<Double>, range: ClosedRange<Double>, label: String,
    onEdit: (() -> Void)? = nil
  ) -> some View {
    VStack(alignment: .leading, spacing: 5) {
      ...
      Slider(value: value, in: range).onChange(of: value.wrappedValue) { _, _ in onEdit?() }
    }
  }
}
